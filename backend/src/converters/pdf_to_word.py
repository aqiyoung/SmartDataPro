...
def convert_pdf_to_word(input_file, output_file=None, options=None):
    ...
    try:
        ...
        logger.info(f"OCR设置: use_ocr={use_ocr}, ocr_lang={ocr_lang}")
        ...
        with pdfplumber.open(input_file) as pdf:
            ...
            for page_num in range(page_count):
                logger.info(f"正在处理第 {page_num + 1}/{page_count} 页")
                ...
                logger.info(f"将第 {page_num + 1} 页转换为图像")
                ...
                logger.info(f"保存页面图像到: {temp_img_path}")
                
                # 2. 执行OCR获取文本
                if use_ocr:
                    logger.info(f"对第 {page_num + 1} 页执行OCR")
                    # 使用新的 OCR 引擎，默认模式
                ...
                # 3. 插入页面图像到Word文档
                logger.info(f"将第 {page_num + 1} 页图像插入到Word文档")
                try:
                    # 直接从临时文件插入图片
                    doc.add_picture(temp_img_path, width=Inches(5.0))
                    doc.add_paragraph()
                    logger.info(f"成功插入第 {page_num + 1} 页图像")
                except Exception as e:
                    logger.info(f"插入图片失败: {str(e)}")
                    import traceback
                    traceback.print_exc()
                ...
                if os.path.exists(temp_img_path):
                    ...
                    logger.info(f"删除临时图像文件: {temp_img_path}")
        ...
        logger.info(f"PDF文件转换成功: {output_file}")
