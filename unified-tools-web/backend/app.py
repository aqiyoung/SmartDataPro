    ...
    try:
        ...
        if thread.is_alive():
            ...
            return Response(
                ...
                headers={
                    "Content-Disposition": f'attachment; filename="转换失败_超时.docx"',
        ...
        # 返回转换后的文件，显式设置Content-Disposition头
        from fastapi.responses import Response
        import os
        
        # 读取文件内容
        print(f"[DEBUG] 读取文件内容: {output_file}")
        with open(output_file, "rb") as f:
            content = f.read()
        print(f"[DEBUG] 文件内容读取完成，大小: {len(content)}字节")
