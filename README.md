...
- **Markdown 专业版优化**：
  ...
  - 支持数学公式渲染（KaTeX）和代码语法高亮
  - 优化标题样式，提供更专业的排版效果
  - 移除字数统计和阅读时间估算功能，保持界面简洁
- **样式主题优化**：
  - 新增 Doocs 经典和 Doocs 优雅主题
  - 优化所有主题的标题样式，采用更现代的设计，匹配 doocs/md 风格
  - 实现一级标题居中显示，采用绿色圆角框样式
  - 移除蓝色元素，使用更中性的颜色方案
  - 实现平滑的动画和过渡效果
- **界面布局优化**：
  - 调整样式控制面板宽度为20%，优化空间分配
  - 实现响应式设计，适配不同屏幕尺寸
  - 优化控件布局，减少留白，提升空间利用率
  - 添加左侧菜单，提供快速导航
- **Bug 修复**：
  - 修复 Markdown 编辑器标题样式应用问题，确保用户编辑内容也能正确显示样式
  - 修复标题在同一行显示的问题，确保每个标题独占一行
  - 修复深色主题下的标题样式一致性问题
...
### v2.2.4 (2026-01-27)
- **UI/UX优化**：
  - 优化首页转换选项卡片的视觉效果，增加卡片大小、改进悬停效果
  - 提升卡片间距和布局通透感，优化整体页面层次感
...
- **代码优化**：
  ...
  - 优化路由配置
- **新增功能**：
  - 添加外部Markdown项目支持，基于GitHub项目 https://github.com/aqiyoung/md
  - 实现独立的Markdown编辑器页面，支持左侧垂直工具栏布局
  - 支持多种主题和格式选项
  - 集成GitHub图床功能，支持图片上传和粘贴
