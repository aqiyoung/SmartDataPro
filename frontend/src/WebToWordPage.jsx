...
const WebToWordPage = () => {
  ...
  const handleConvert = async (e) => {
    ...
    try {
      if (!file && !url) {
        throw new Error('请选择要转换的文件或输入网页URL');
      }
      ...
      formData.append('conversion_type', 'web-to-docx');

      const response = await fetch('/api/convert/web-to-docx', {
        method: 'POST',
        body: formData
      });
      ...
      if (contentType && contentType.includes('application/vnd.openxmlformats-officedocument.wordprocessingml.document')) {
        ...
        a.href = url;
        // 从响应头中获取文件名，或者使用默认文件名
        const contentDisposition = response.headers.get('content-disposition');
        let fileName = '网页转换结果.docx';
        ...
        setResult({ success: true, message: '转换成功' });
      } else {
        // 处理JSON响应（如果后端返回错误信息）
      ...
      <main className="app-main" style={{ position: 'relative', textAlign: 'center', minHeight: '80vh', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        ...
        <div style={{ maxWidth: '800px', margin: '0 auto', padding: '2rem', width: '100%' }}>
          ...
          <div style={{ marginBottom: '3rem', animation: 'fadeIn 0.6s ease-out' }}>
            <h3 style={{ fontSize: '2rem', fontWeight: '700', color: '#667eea', marginBottom: '1rem', animation: 'slideUp 0.5s ease-out' }}>网页转 Word</h3>
            <p style={{ fontSize: '1.1rem', color: '#666', maxWidth: '600px', margin: '0 auto', lineHeight: '1.5', animation: 'slideUp 0.5s ease-out 0.1s both' }}>将网页内容转换为Word文档，支持普通网页和微信公众号文章</p>
          </div>
          ...
          <form className="conversion-form" onSubmit={handleConvert} style={{ animation: 'fadeIn 0.6s ease-out 0.2s both' }}>
            <div style={{ backgroundColor: 'white', borderRadius: '24px', boxShadow: '0 8px 32px rgba(0, 0, 0, 0.1)', padding: '3rem', transition: 'all 0.3s ease' }}>
              ...
              {/* 文件上传区域 */}
              <div style={{ marginBottom: '2.5rem' }}>
                <p style={{ marginBottom: '1rem', color: '#666', fontSize: '1rem' }}>或者选择HTML文件:</p>
                <button 
                  type="button" 
                  style={{
              ...
              <div style={{ display: 'flex', gap: '1.5rem', justifyContent: 'center', flexWrap: 'wrap' }}>
                <button 
                  ...
                  style={{
                    ...
                    transition: 'all 0.3s ease',
                    minWidth: '160px',
                    backgroundColor: isLoading || (!file && !url) ? '#a5b4fc' : '#667eea',
                    color: 'white',
                    boxShadow: isLoading || (!file && !url) ? 'none' : '0 4px 16px rgba(102, 126, 234, 0.3)',
                    opacity: isLoading || (!file && !url) ? 0.7 : 1
                  }}
                  disabled={isLoading || (!file && !url)}
                  onMouseEnter={(e) => {
                    if (!isLoading && (file || url)) {
